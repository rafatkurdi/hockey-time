...
@Component({
  selector: 'app-slot-shots-saved',
  templateUrl: './slot-shots-saved.component.html',
  styleUrls: ['./slot-shots-saved.component.scss']
})
export class SlotShotsSavedComponent implements OnInit {

  @Output() saveToPlaylist = new EventEmitter<CommonObject>();
  @Output() playSlotShotsSaved = new EventEmitter<VideToPlay[]>();
  ...
  playOne(goal: CommonObject) {
    ...
    this.playSlotShotsSaved.emit(video);
  }
}
