...
@Component({
  selector: 'app-inner-slot-shot-saved',
  templateUrl: './inner-slot-shot-saved.component.html',
  styleUrls: ['./inner-slot-shot-saved.component.scss']
})
export class InnerSlotShotSavedComponent implements OnInit {

  @Output() saveToPlaylist = new EventEmitter<CommonObject>();
  @Output() playInnerSlotShotSaved = new EventEmitter<VideToPlay[]>();
  ...
  playOne(goal: CommonObject) {
    ...
    this.playInnerSlotShotSaved.emit(video);
  }
}
